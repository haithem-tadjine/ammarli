...
import { 
  ...
  Map, ShieldAlert, FileText, ChevronRight, Menu, LayoutDashboard, UserPlus, PieChart
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <div className={`sidebar glass-panel ${collapsed ? 'collapsed' : ''}`} style={{
      ...
      <div className="flex items-center justify-between" style={{ padding: '24px 16px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <button onClick={() => setCollapsed(!collapsed)} style={{ background: 'transparent', border: 'none', color: 'var(--text-sidebar)', cursor: 'pointer' }}>
          {collapsed ? <Menu size={24} /> : <ChevronRight size={24} />}
        </button>
      </div>
